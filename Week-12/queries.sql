...
select max(mycount) from (select count(wId)as mycount from store group by wId);
...
select * from (select * from customer inner join orders where customer.cNo= orders.cNo) as customerOrders inner join orderItem where customerOrders.oId=orderItem.oId;

select customer.cNo, cName, oId, oDate from customer inner join orders where customer.cNo= orders.cNo;
